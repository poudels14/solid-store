...
const STORE_REFS = new Map<Symbol, any>();

type StoreRef = Record<keyof any, any> & {
  [$PREV]?: any;
  [$GET]?: () => any;
  [$SET]?: (v: any) => void;
};
...
type Store<T> = T & StoreRef;
type StoreSignal<T> = StoreRef & (() => Store<T>);

type StoreSetter<T> =
  | ((field: string, value: Store<T>) => void)
  | ((value: Store<T>) => void);
...
function createStore<T>(init: T) {
  ...
  }) as unknown as StoreRef;

  STORE_REFS.set(root, trap(init));

  return [store, storeUpdater(root)] as [RootStore<T>, StoreSetter<T>];
}

const trap = (prev: any) => {
  let target: StoreRef = Object.defineProperty(function () {}, $PREV, {
    value: prev,
    writable: true,
...
function storeRef<T>(store: Store<unknown>) {
  const root = store[$STORE];
  if (!root) {
    throw new Error(
      `"useStore" can only be used with stores. Using with:` +
...
function storeUpdater(root: Symbol) {
  return (...path: any[]) => {
    batch(() => {
...
const compareAndNotify = (ref: any, value: any) => {
  const prev = ref[$PREV];
  if (prev === value) {
    return;
  }
